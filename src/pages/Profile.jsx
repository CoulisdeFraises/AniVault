  ...
  return (
    <div className="space-y-1 animate-fadeIn">
      <div className="flex gap-1">
        {levels.map((_, i) => (
          <div key={i} className={`flex-1 h-1 rounded-full transition-all duration-300 motion-reduce:transition-none ${i < score ? lvl.color : "bg-white/10"}`} />
        ))}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-sm rounded-2xl bg-violet-900 border border-white/10 p-6 space-y-4 animate-fadeInUp shadow-2xl">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="font-semibold text-violet-50" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Confirmer le changement</p>
            <p className="text-sm text-violet-400 mt-1">Tu es sur le point de modifier ton mot de passe. Cette action est immédiate et irréversible.</p>
          </div>
        </div>
...
function PasswordSection({ userEmail }) {
  ...
  function reset() {
    ...
    setMsg({ type: "", text: "" }); setShowConfirmModal(false);
  }
  function handleClose() { reset(); setOpen(false); }

  function handleRequestChange() {
    setMsg({ type: "", text: "" });
    if (!currentPassword) { setMsg({ type: "error", text: "Saisis ton mot de passe actuel." }); return; }
    if (newPassword.length < 8) { setMsg({ type: "error", text: "Le nouveau mot de passe doit faire au moins 8 caractères." }); return; }
    if (newPassword === currentPassword) { setMsg({ type: "error", text: "Le nouveau mot de passe doit être différent de l'actuel." }); return; }
  ...
  async function handleConfirmChange() {
    ...
    if (authError) {
      setSaving(false); setShowConfirmModal(false);
...
// ── Succès ────────────────────────────────────────────────────────────────────
const TIER_TEXT   = { bronze: "text-amber-700",  silver: "text-slate-400",  gold: "text-amber-400"  };
const TIER_BORDER = { bronze: "border-amber-700/60", silver: "border-slate-400/60", gold: "border-amber-400/60" };
...
function AchievementRow({ achievement, unlocked }) {
  return (
    <div className={`flex items-center gap-3 px-4 py-2.5 border-b border-white/5 last:border-0 transition-opacity motion-reduce:transition-none ${unlocked ? "opacity-100" : "opacity-35"}`}>
      ...
      {unlocked ? (
        <span className={`flex-shrink-0 font-mono text-[9px] uppercase tracking-widest px-1.5 py-0.5 rounded-full border ${TIER_TEXT[achievement.tier]} ${TIER_BORDER[achievement.tier]}`}>{achievement.tier}</span>
      ) : (
      ...
      <button onClick={() => setOpen((v) => !v)}
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="w-16 h-1.5 rounded-full bg-white/10 overflow-hidden">
            <div className={`h-full rounded-full transition-all duration-500 motion-reduce:transition-none ${allDone ? "bg-amber-400" : "bg-violet-500"}`} style={{ width: `${pct}%` }} />
          </div>
          <span className={`font-mono text-[10px] tabular-nums ${allDone ? "text-amber-400" : "text-violet-400"}`}>{unlocked}/{total}</span>
        </div>
      </button>
      {open && (
        <div className="bg-violet-950/30 animate-fadeIn">
          {achievements.map((a) => <AchievementRow key={a.id} achievement={a} unlocked={unlockedIds.has(a.id)} />)}
        </div>
      )}
    </div>
  );
    ...
    <div className="flex flex-col">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5">
        <div className="flex-1 h-1.5 rounded-full bg-white/10 overflow-hidden">
          <div className="h-full rounded-full bg-amber-400 transition-all duration-700 motion-reduce:transition-none" style={{ width: `${globalPct}%` }} />
        </div>
        <span className="font-mono text-[11px] text-violet-400 flex-shrink-0 tabular-nums">{unlockedCount}/{total} · {globalPct} %</span>
      </div>
...
export function Profile() {
  ...
  const daysLeft = (() => {
    if (!userProfile?.username_last_changed) return 0;
    const ms = Date.now() - new Date(userProfile.username_last_changed).getTime();
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      setProfileMsg({ type: "error", text: e.message || "Impossible de sauvegarder." });
    }
    setSavingProfile(false);
    setTimeout(() => setProfileMsg({ type: "", text: "" }), 3500);
  }
  ...
  async function handleResetLibrary() {
    ...
    try {
      await setEntries([]);
      setResetMsg({ type: "success", text: "Liste réinitialisée avec succès !" });
      setTimeout(() => { setResetMsg({ type: "", text: "" }); setConfirmReset(false); }, 2500);
    } catch {
      setResetMsg({ type: "error", text: "Une erreur est survenue. Réessaie." });
    } finally {
      setResetting(false);
    }
  }
            ...
            {!confirmReset ? (
              <>
                <button onClick={() => setConfirmReset(true)}
                  ...
                  <Trash2 size={15} /> Réinitialiser ma liste
                </button>
                <p className="text-[11px] text-violet-500 mt-1">Efface tous les animes et séries ajoutés. Ton compte et tes succès sont conservés.</p>
              </>
            ) : (
              <div className="space-y-3 animate-fadeIn">
                <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-400/10 border border-amber-400/20">
                  <AlertTriangle size={16} className="text-amber-400 flex-shrink-0 mt-0.5" />
                  <p className="text-sm text-violet-200">Toute ta bibliothèque sera <span className="text-amber-300 font-semibold">définitivement effacée</span>. Ton compte et tes succès seront conservés.</p>
                </div>
                <Msg {...resetMsg} />
                <div className="flex gap-2">
                  <button onClick={() => { setConfirmReset(false); setResetMsg({ type: "", text: "" }); }} disabled={resetting}
                    className="flex-1 px-4 py-2 rounded-xl text-sm font-medium bg-white/10 text-violet-200 hover:bg-white/20 disabled:opacity-50 transition-colors motion-reduce:transition-none">Annuler</button>
                  <button onClick={handleResetLibrary} disabled={resetting}
                    ...
                    {resetting ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />} Vider la liste
                  </button>
                </div>
              </div>
            )}
          ...
          <div className="px-5 py-4">
            {!confirmDelete ? (
              <>
                ...
                <p className="text-[11px] text-violet-500 mt-1">Supprime définitivement ton compte et toutes tes données. Irréversible.</p>
              </>
            ) : (
              <div className="space-y-3 animate-fadeIn">
                <p className="text-sm text-violet-200">Es-tu sûr ? Ton compte et toute ta bibliothèque seront <span className="text-rose-300 font-semibold">définitivement supprimés</span>.</p>
                <div className="flex gap-2">
