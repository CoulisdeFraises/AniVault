...
export function TitleFormModal({ editingEntry, onClose }) {
  ...
  async function handleSelectResult(result) {
    ...
    } finally { setImporting(false); setSearchQuery(""); setSearchOpen(false); }
  }
    ...
    <>
      <Modal onClose={onClose}>
        <form onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit} className="p-6">
          ...
          {importing && <p className="text-[11px] text-violet-400 mb-2">Import de la franchise en cours…</p>}

          <label className="block text-xs uppercase tracking-wide text-violet-400 mb-1">Titre</label>
          <input value={form.title} onChange={(e) => setForm((f) => ({ ...f, title: e.target.value }))} placeholder="ex : Frieren, Severance…"
          ...
          {!importedFrom && !editingId && <p className="text-[11px] text-violet-500 mt-2">Les saisons se gèrent depuis la fiche une fois le titre ajouté.</p>}
          ...
          <div className="mt-3">
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              ...
              {form.genres.filter((g) => !GENRE_SUGGESTIONS.includes(g)).map((g) => <Chip key={g} active onClick={() => toggleGenre(g)} colorClass="bg-white/20 border-white/30">{g}</Chip>)}
            </div>
            <div className="flex gap-2">
              <input value={genreInput} onChange={(e) => setGenreInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addCustomGenre(); } }} placeholder="Ajouter un genre personnalisé…"
          ...
          {editingId && (
            <>
              ...
              <div className="mt-3">
                ...
                <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} rows={3} placeholder="Ce que tu en as pensé…"
      ...
      {duplicateWarning && (
        <ConfirmDialog
          icon={<span className="text-amber-400 text-sm font-bold">!</span>} tone="amber" title="Titre déjà présent"
          description={
            <>
              ...
              {duplicateWarning.seasons && <span className="block mt-2 text-[11px] font-mono text-violet-400">{duplicateWarning.seasons.length} saison{duplicateWarning.seasons.length > 1 ? "s" : ""} · {seasonTotals(duplicateWarning.seasons).watched} épisodes vus</span>}
            </>
          }
