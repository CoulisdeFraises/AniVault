...
export function TitleFormModal({ editingEntry, onClose }) {
  ...
  const [form, setForm] = useState(() => editingEntry ? { ...editingEntry, seasons: editingEntry.seasons.map((s) => ({ ...s })) } : emptyForm);
        ...
        <form onClick={(e) => e.stopPropagation()} onSubmit={handleSubmit} className="p-6">
          ...
          {importedFrom && (
            <p className="text-[11px] text-teal-300 mb-2 flex items-center gap-1">
              <Check size={12} /> Importé depuis {importedFrom} — {form.seasons.length} saison{form.seasons.length > 1 ? "s" : ""} détectée{form.seasons.length > 1 ? "s" : ""}.
            </p>
          )}
          ...
          <div className="mt-3">
            ...
            <select value={form.status} onChange={(e) => setForm((f) => ({ ...f, status: e.target.value }))} className="w-full px-3 py-2 rounded-lg bg-violet-950/60 border border-white/10 text-violet-50 focus:outline-none focus:ring-2 focus:ring-amber-400">
              {STATUS_ORDER.map((k) => <option key={k} value={k}>{STATUS[k].label}</option>)}
            </select>
          </div>

          {!importedFrom && !editingId && (
            <p className="text-[11px] text-violet-500 mt-2">Les saisons se gèrent depuis la fiche une fois le titre ajouté (bouton "+ Saison").</p>
          )}

          <div className="mt-3">
            <label className="block text-xs uppercase tracking-wide text-violet-400 mb-1">Genres</label>
            <div className="flex flex-wrap gap-1.5 mb-2">
              {GENRE_SUGGESTIONS.map((g) => <Chip key={g} active={form.genres.includes(g)} onClick={() => toggleGenre(g)} colorClass="bg-white/20 border-white/30">{g}</Chip>)}
            ...
            <div className="flex gap-2">
              <input value={genreInput} onChange={(e) => setGenreInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); addCustomGenre(); } }} placeholder="Ajouter un genre personnalisé…" className="flex-1 px-3 py-1.5 rounded-lg bg-violet-950/60 border border-white/10 text-violet-50 placeholder-violet-500 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
          ...
          <div className="mt-3">
            ...
            <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} rows={3} placeholder="Ce que tu en as pensé…" className="w-full px-3 py-2 rounded-lg bg-violet-950/60 border border-white/10 text-violet-50 placeholder-violet-500 text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
          </div>
          ...
          <div className="flex justify-end gap-2 mt-5">
            ...
            <button type="submit" className="px-4 py-2 rounded-lg text-sm font-semibold bg-amber-400 text-violet-950 hover:bg-amber-300">{editingId ? "Enregistrer" : "Ajouter"}</button>
          </div>
        </form>
      </Modal>
