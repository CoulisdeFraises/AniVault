...
const STATUS_CHIP_COLOR = { "en-cours":"bg-amber-400/90 border-amber-400 text-violet-950","termine":"bg-teal-400/90 border-teal-400 text-violet-950","a-voir":"bg-sky-400/90 border-sky-400 text-violet-950","abandonne":"bg-rose-400/90 border-rose-400 text-violet-950" };
const FORMAT_CHIP_COLOR = { tv:"bg-violet-600 border-violet-500 text-white",ova:"bg-purple-600 border-purple-500 text-white",movie:"bg-amber-600 border-amber-500 text-white" };

export function Header({ typeFilter,selectedStatuses=[],selectedFormats=[],searchQuery="",onTypeFilterChange,onToggleStatus,onToggleFormat,onClearFilters,onSearchChange,onAddClick,syncing=false,syncProgress={current:0,total:0},onSyncClick }) {
  const { entries, loading } = useLibrary();
  const searchRef = useRef(null);
  const [searchFocused, setSearchFocused] = useState(false);

  useEffect(()=>{
    function h(e){ if(e.key==="/"&&document.activeElement?.tagName!=="INPUT"&&document.activeElement?.tagName!=="TEXTAREA"){e.preventDefault();searchRef.current?.focus();} }
  ...
  const hasFilters   = selectedStatuses.length>0||selectedFormats.length>0;
  const showFmt      = typeFilter==="anime";
    ...
    <>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={onSyncClick} disabled={syncing}
              ...
              {syncing?<span className="text-xs font-mono text-violet-400 hidden sm:inline">{syncProgress.current}/{syncProgress.total}</span>:<span className="text-xs font-mono text-violet-400 hidden sm:inline">Sync</span>}
            </button>
            <BurgerMenu/>
            <button onClick={onAddClick}
              className="flex items-center gap-1.5 bg-amber-400 text-violet-950 font-semibold text-sm px-4 py-2 rounded-xl hover:bg-amber-300 active:scale-95 transition-all motion-reduce:transition-none focus:outline-none focus-visible:ring-2 focus-visible:ring-amber-300">
              <Plus size={16}/>
      ...
      {!loading&&entries.length>0&&(
        <div className="rounded-2xl bg-violet-900/30 border border-white/5 mb-6 overflow-hidden">
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-white/10">
            ...
            <div className="p-4">{topGenres.length>0?(<><p className="font-medium truncate">{topGenres.map(([g])=>g).join(", ")}</p><p className="text-[11px] text-violet-400 uppercase tracking-wide">Genres préférés</p></>):(<><p className="font-medium text-violet-500">—</p><p className="text-[11px] text-violet-400 uppercase tracking-wide">Genres préférés</p></>)}</div>
          </div>
          {totalKnown>0&&(
      ...
      <div className="mb-4">
        <div className={`flex items-center gap-2 px-4 py-2.5 rounded-2xl border transition-colors duration-200 motion-reduce:transition-none ${searchFocused?"bg-violet-900/60 border-violet-500/60 shadow-[0_0_0_3px_rgba(139,92,246,0.15)]":"bg-violet-900/30 border-white/5 hover:border-white/10"}`}>
          ...
          <input ref={searchRef} type="text" value={searchQuery} onChange={e=>onSearchChange(e.target.value)} onFocus={()=>setSearchFocused(true)} onBlur={()=>setSearchFocused(false)}
          ...
          {isSearch&&(<button onClick={()=>{onSearchChange("");searchRef.current?.focus();}} aria-label="Effacer" className="flex-shrink-0 p-0.5 rounded-full text-violet-400 hover:text-violet-200 hover:bg-white/10 active:scale-90 transition-all motion-reduce:transition-none"><X size={14}/></button>)}
          {!isSearch&&!searchFocused&&(<kbd className="hidden sm:inline-flex items-center px-1.5 py-0.5 rounded border border-white/10 font-mono text-[10px] text-violet-500 select-none">/</kbd>)}
        </div>
      </div>

      {/* ══ BOUTON AJOUTER (proéminent, au-dessus des filtres) ════════════ */}
      <button onClick={onAddClick}
      ...
      <div className="rounded-2xl bg-violet-900/20 border border-white/5 p-4 mb-5 space-y-3">
        <div className="flex items-center justify-between">
          ...
          {hasFilters&&(<button onClick={onClearFilters} className="flex items-center gap-1 text-[10px] font-mono text-violet-400 hover:text-violet-200 transition-colors motion-reduce:transition-none"><X size={10}/> Réinitialiser</button>)}
        </div>
        <div>
          <p className="font-mono text-[10px] uppercase tracking-wider text-violet-500 mb-2">Statut</p>
          <div className="flex flex-wrap gap-1.5">
            {STATUS_ORDER.map(k=><FilterChip key={k} active={selectedStatuses.includes(k)} onClick={()=>onToggleStatus(k)} colorClass={STATUS_CHIP_COLOR[k]}><span className={`h-1.5 w-1.5 rounded-full flex-shrink-0 ${STATUS[k].dot}`}/>{STATUS[k].label}</FilterChip>)}
          </div>
        </div>
        {showFmt&&(
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {Object.entries(CATEGORY_LABELS).map(([key,label])=><FilterChip key={key} active={selectedFormats.includes(key)} onClick={()=>onToggleFormat(key)} colorClass={FORMAT_CHIP_COLOR[key]}>{CATEGORY_ICONS[key]} {label}</FilterChip>)}
            </div>
          </div>
        )}
      </div>
